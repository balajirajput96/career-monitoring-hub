...
- [x] Design a durable, reviewable reel-production queue and checkpoint data model that tracks unique topic identity, research evidence, production state, QC outcome, and verified Drive delivery without fabricating completion. Added settings, item-ledger, and run-history tables with unique user/reel and user/topic constraints.
- [x] Implement an owner-scoped Reel Production dashboard with batch progress, next-reel state, evidence/QC status, and safe manual controls while preserving the existing Career Hub screens. Added protected `reels` procedures and the `/reels` owner route; the promoted owner ledger is seeded only with the five Drive-verified package records.
- [x] Configure a bounded daily continuation that resumes from the verified queue checkpoint, never duplicates completed Drive packages, and records a clear blocker when generation or Drive verification fails. Existing task `VRAoDCLjaeUeaW3dFez5PY` is active daily at 10:00 Asia/Calcutta through 2035-01-01 and retains Google Workspace access.
- [x] Fix and live-verify the published `/reels` route so the owner-scoped Reel Production dashboard is reachable after deployment rather than returning a 404. Live deployment now resolves the protected route and correctly shows its sign-in boundary when no owner session is present.
- [x] Record the verified Reel 0006 Drive delivery in the persistent owner ledger, advance the queue only after package verification, and preserve the source/QC evidence. The package is listed in Drive with the MP4, Hindi narration, original music, captions, research brief, renderer, QC, metadata, and delivery verification; the owner ledger is delivered/verified and next reel is 0007.
- [x] Repair and test the Reel Production run-status mapping against the deployed `workflowRunStatus` database column. The existing shared Drizzle enum builder already maps this physical column correctly; a focused regression assertion now locks that contract and passed.
- [x] Produce, quality-check, and Drive-verify the next unique Reel 0007 on self-distancing and emotional perspective; do not update the ledger until the complete package is listed in Drive. The 60.23-second 9:16 Hindi package is delivered/verified in Drive with sources, captions, narration, original music, QC, checksum, anchor visual, renderer, and delivery manifest; queue advances to Reel 0008.
- [ ] Prepare and complete the next unique Reel 0008 on affect labeling—naming a feeling—with research provenance, technical QC, and verified Drive delivery; retain it as pending until its complete package is listed.
