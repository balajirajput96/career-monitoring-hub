...
export default function Home() {
  ...
  const draftCoverNote = trpc.career.applications.draftCoverNote.useMutation({ onSuccess: () => { toast.success("Truthful cover-note draft saved for review"); utils.career.overview.invalidate(); }, onError: error => toast.error(error.message) });
  const updateContact = trpc.career.contacts.update.useMutation({ onSuccess: () => { toast.success("Recruiter contact updated"); utils.career.overview.invalidate(); }, onError: error => toast.error(error.message) });
  const addContact = trpc.career.contacts.add.useMutation({ onSuccess: () => { toast.success("Recruiter contact saved"); utils.career.overview.invalidate(); setContactForm({ name: "", company: "", role: "", email: "", linkedInUrl: "", notes: "" }); }, onError: error => toast.error(error.message) });
  const recruiterEmailEvents = trpc.career.contacts.emailEvents.useQuery();
  const ingestRecruiterEmail = trpc.career.contacts.ingestEmailEvents.useMutation({
    onSuccess: () => {
      toast.success("Email processed and available in the review queue.");
      utils.career.contacts.emailEvents.invalidate();
      utils.career.contacts.list.invalidate();
      utils.career.overview.invalidate();
      setRecruiterEmailForm({ messageId: "", threadId: "", sender: "", subject: "", receivedAt: "", snippet: "" });
    },
    onError: error => toast.error(error.message),
  });
  const reviewRecruiterEmail = trpc.career.contacts.reviewEmailEvent.useMutation({
    onSuccess: () => { toast.success("Email review status saved. No reply was sent."); utils.career.contacts.emailEvents.invalidate(); },
    onError: error => toast.error(error.message),
  });
  ...
  const [contactForm, setContactForm] = useState({ name: "", company: "", role: "", email: "", linkedInUrl: "", notes: "" });
  const [recruiterEmailForm, setRecruiterEmailForm] = useState({ messageId: "", threadId: "", sender: "", subject: "", receivedAt: "", snippet: "" });
  ...
  const { metrics, schedule, jobs, applications, pendingApprovals, reports, runs, sources, recruiterContacts } = overview.data;
      ...
      <div className="mx-auto max-w-7xl space-y-6 pb-12">
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <aside className="space-y-6">
            ...
            <section className="soft-panel rounded-3xl border border-white/80 p-5"><div className="flex items-center gap-2"><FileText className="h-4 w-4 text-primary" /><h2 className="font-semibold text-slate-900">Latest daily report</h2></div>{reports[0] ? (reports[0].contentEnglish || reports[0].contentHindi ? <div className="mt-3 space-y-3"><div className="rounded-2xl bg-slate-50 p-3"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">English</p><p className="mt-1 whitespace-pre-line text-sm leading-6 text-slate-600">{reports[0].contentEnglish ?? "Not available for this earlier report."}</p></div><div className="rounded-2xl bg-slate-50 p-3"><p className="text-xs font-semibold tracking-[0.08em] text-slate-500">हिंदी</p><p className="mt-1 whitespace-pre-line text-sm leading-6 text-slate-600">{reports[0].contentHindi ?? "इस पुराने रिपोर्ट के लिए उपलब्ध नहीं है।"}</p></div><p className="text-xs text-slate-500">Generated {formatDate(reports[0].createdAt)} · Primary language: {reports[0].language === "hi" ? "Hindi" : "English"}</p></div> : <><p className="mt-3 whitespace-pre-line text-sm leading-6 text-slate-600">{reports[0].content}</p><p className="mt-4 text-xs text-slate-500">Generated {formatDate(reports[0].createdAt)} · {reports[0].language === "hi" ? "Hindi" : "English"}</p></>) : <p className="mt-3 text-sm leading-6 text-slate-500">Your bilingual end-of-run report will appear here after the next scheduled discovery.</p>}</section>
          </aside>
        </section>
        ...
        <section className="grid gap-6 xl:grid-cols-2">
          <div className="soft-panel rounded-3xl border border-white/80 p-5 sm:p-6"><div className="flex items-center justify-between"><div><div className="flex items-center gap-2"><BriefcaseBusiness className="h-4 w-4 text-primary" /><h2 className="font-semibold text-slate-900">Application tracker</h2></div><p className="mt-1 text-sm text-slate-500">Persistent status, notes and follow-up context.</p></div><Badge variant="outline" className="border-slate-200 bg-white">{applications.length} records</Badge></div><div className="mt-5 space-y-3">{applications.length === 0 ? <p className="rounded-2xl border border-dashed border-slate-200 bg-white/70 p-6 text-center text-sm text-slate-500">Shortlist a verified job to begin tracking it.</p> : applications.slice(0, 6).map(item => <div key={item.application.id} className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white/80 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0 flex-1"><p className="truncate font-medium text-slate-900">{item.job.title}</p><p className="mt-1 text-sm text-slate-500">{item.job.company} · {item.match?.overallScore ?? 0} match</p>{item.application?.coverNoteDraft ? <p className="mt-2 line-clamp-2 text-xs text-slate-500">Draft saved: {item.application.coverNoteDraft}</p> : null}<div className="mt-3 grid gap-2 sm:grid-cols-3"><Select value={item.application.resumeVersion ?? "none"} onValueChange={value => updateApplication.mutate({ jobId: item.job.id, status: item.application.status, resumeVersion: value === "none" ? undefined : value })}><SelectTrigger className="h-8 bg-white text-xs"><SelectValue placeholder="Resume version" /></SelectTrigger><SelectContent><SelectItem value="none">No resume selected</SelectItem>{profileForm.resumeVersions.map(resume => <SelectItem key={resume.storageKey ?? resume.name} value={resume.storageKey ?? resume.name}>{resume.name}</SelectItem>)}</SelectContent></Select><Input type="date" className="h-8 bg-white text-xs" defaultValue={item.application.followUpAt ? new Date(item.application.followUpAt).toISOString().slice(0, 10) : ""} onBlur={event => updateApplication.mutate({ jobId: item.job.id, status: item.application.status, followUpAt: event.target.value ? new Date(`${event.target.value}T09:00:00+05:30`) : undefined })} aria-label="Follow-up date" /><Input className="h-8 bg-white text-xs" defaultValue={item.application.notes ?? ""} onBlur={event => updateApplication.mutate({ jobId: item.job.id, status: item.application.status, notes: event.target.value })} placeholder="Follow-up notes" /></div></div><Select value={item.application.status} onValueChange={value => updateApplication.mutate({ jobId: item.job.id, status: value as "found" | "shortlisted" | "approval_pending" | "applied" | "rejected" | "follow_up" | "closed" })}><SelectTrigger className="h-9 w-[158px] bg-white"><SelectValue /></SelectTrigger><SelectContent>{["found", "shortlisted", "approval_pending", "applied", "follow_up", "rejected", "closed"].map(status => <SelectItem key={status} value={status}>{status.replaceAll("_", " ")}</SelectItem>)}</SelectContent></Select></div>)}</div></div>
        ...
        <section className="soft-panel rounded-3xl border border-white/80 p-5 sm:p-6">
          <div className="flex items-center justify-between"><div><div className="flex items-center gap-2"><UserRound className="h-4 w-4 text-primary" /><h2 className="font-semibold text-slate-900">Verified recruiter contacts</h2></div><p className="mt-1 text-sm text-slate-500">Only contacts you explicitly record are stored. No message is sent automatically.</p></div><Badge variant="outline" className="border-slate-200 bg-white">{recruiterContacts.length} records</Badge></div>
          <form className="mt-5 grid gap-2 rounded-2xl border border-slate-100 bg-white/70 p-3 sm:grid-cols-2" onSubmit={event => { event.preventDefault(); addContact.mutate({ name: contactForm.name, company: contactForm.company || undefined, role: contactForm.role || undefined, email: contactForm.email || undefined, linkedInUrl: contactForm.linkedInUrl || undefined, notes: contactForm.notes || undefined }); }}><Input required value={contactForm.name} onChange={event => setContactForm(current => ({ ...current, name: event.target.value }))} placeholder="Contact name" aria-label="Contact name" /><Input value={contactForm.company} onChange={event => setContactForm(current => ({ ...current, company: event.target.value }))} placeholder="Company" aria-label="Company" /><Input value={contactForm.role} onChange={event => setContactForm(current => ({ ...current, role: event.target.value }))} placeholder="Role" aria-label="Role" /><Input type="email" value={contactForm.email} onChange={event => setContactForm(current => ({ ...current, email: event.target.value }))} placeholder="Public email (optional)" aria-label="Public email" /><Input type="url" value={contactForm.linkedInUrl} onChange={event => setContactForm(current => ({ ...current, linkedInUrl: event.target.value }))} placeholder="Public LinkedIn URL (optional)" aria-label="Public LinkedIn URL" /><Input value={contactForm.notes} onChange={event => setContactForm(current => ({ ...current, notes: event.target.value }))} placeholder="Notes / follow-up context" aria-label="Contact notes" /><Button type="submit" className="sm:col-span-2" disabled={addContact.isPending}><Plus className="mr-1.5 h-4 w-4" />{addContact.isPending ? "Saving…" : "Add verified contact"}</Button></form><div className="mt-5 grid gap-3 md:grid-cols-2">{recruiterContacts.length === 0 ? <p className="rounded-2xl border border-dashed border-slate-200 bg-white/70 p-6 text-center text-sm text-slate-500 md:col-span-2">No recruiter contacts recorded yet. Add verified public contact details through the contact API when available.</p> : recruiterContacts.slice(0, 8).map(contact => <div key={contact.id} className="rounded-2xl border border-slate-100 bg-white/80 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium text-slate-900">{contact.name}</p><p className="mt-1 text-sm text-slate-500">{contact.company ?? "Company not recorded"}{contact.role ? ` · ${contact.role}` : ""}</p></div><Select value={contact.responseStatus} onValueChange={value => updateContact.mutate({ contactId: contact.id, responseStatus: value as "discovered" | "contacted" | "replied" | "no_response" | "closed" })}><SelectTrigger className="h-8 w-[130px] bg-white text-xs"><SelectValue /></SelectTrigger><SelectContent>{["discovered", "contacted", "replied", "no_response", "closed"].map(status => <SelectItem key={status} value={status}>{status.replaceAll("_", " ")}</SelectItem>)}</SelectContent></Select></div>{contact.email ? <p className="mt-3 text-xs text-slate-500">{contact.email}</p> : null}{contact.linkedInUrl ? <a className="mt-1 block truncate text-xs text-primary hover:underline" href={contact.linkedInUrl} target="_blank" rel="noreferrer">Open public LinkedIn profile</a> : null}<Input className="mt-3 h-8 bg-white text-xs" defaultValue={contact.notes ?? ""} placeholder="Edit contact notes" aria-label={`Notes for ${contact.name}`} onBlur={event => updateContact.mutate({ contactId: contact.id, notes: event.target.value })} /><Input type="date" className="mt-2 h-8 bg-white text-xs" defaultValue={contact.lastContactAt ? new Date(contact.lastContactAt).toISOString().slice(0, 10) : ""} aria-label={`Follow-up date for ${contact.name}`} onBlur={event => updateContact.mutate({ contactId: contact.id, lastContactAt: event.target.value ? new Date(`${event.target.value}T09:00:00+05:30`) : undefined })} /></div>)}</div>
        </section>

        <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <form className="soft-panel rounded-3xl border border-white/80 p-5 sm:p-6" onSubmit={event => {
            event.preventDefault();
            ingestRecruiterEmail.mutate({
              events: [buildRecruiterEmailImportEvent(recruiterEmailForm)],
            });
          }}>
            <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><FileText className="h-4 w-4 text-primary" /><h2 className="font-semibold text-slate-900">Import recruiter email</h2></div><p className="mt-1 text-sm text-slate-500">OAuth-free manual capture. It records only email metadata and never sends a reply.</p></div><Badge variant="outline" className="border-amber-200 bg-amber-50 text-amber-800">Review first</Badge></div>
            <div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="sm:col-span-2"><Label htmlFor="email-message-id">Stable message ID</Label><Input id="email-message-id" required className="mt-1.5 bg-white" value={recruiterEmailForm.messageId} onChange={event => setRecruiterEmailForm(current => ({ ...current, messageId: event.target.value }))} placeholder="Gmail/RFC message ID — used to block duplicates" /></div><div><Label htmlFor="email-sender">Sender email</Label><Input id="email-sender" required type="email" className="mt-1.5 bg-white" value={recruiterEmailForm.sender} onChange={event => setRecruiterEmailForm(current => ({ ...current, sender: event.target.value }))} placeholder="recruiter@company.com" /></div><div><Label htmlFor="email-received">Received at <span className="text-muted-foreground">(optional)</span></Label><Input id="email-received" type="datetime-local" className="mt-1.5 bg-white" value={recruiterEmailForm.receivedAt} onChange={event => setRecruiterEmailForm(current => ({ ...current, receivedAt: event.target.value }))} /></div><div className="sm:col-span-2"><Label htmlFor="email-subject">Subject</Label><Input id="email-subject" required className="mt-1.5 bg-white" value={recruiterEmailForm.subject} onChange={event => setRecruiterEmailForm(current => ({ ...current, subject: event.target.value }))} placeholder="Interview update / recruiter response" /></div><div className="sm:col-span-2"><Label htmlFor="email-thread">Thread ID <span className="text-muted-foreground">(optional)</span></Label><Input id="email-thread" className="mt-1.5 bg-white" value={recruiterEmailForm.threadId} onChange={event => setRecruiterEmailForm(current => ({ ...current, threadId: event.target.value }))} placeholder="Paste a stable thread ID when available" /></div><div className="sm:col-span-2"><Label htmlFor="email-snippet">Short snippet <span className="text-muted-foreground">(optional)</span></Label><Textarea id="email-snippet" className="mt-1.5 min-h-24 bg-white" value={recruiterEmailForm.snippet} onChange={event => setRecruiterEmailForm(current => ({ ...current, snippet: event.target.value }))} placeholder="Paste only the necessary response context; avoid passwords, IDs, or other sensitive data." /></div></div>
            <p className="mt-3 text-xs leading-5 text-slate-500">The sender is matched to an existing verified contact when possible. Imported events remain unreviewed until you explicitly mark them reviewed or ignored.</p><Button type="submit" className="mt-5" disabled={ingestRecruiterEmail.isPending}>{ingestRecruiterEmail.isPending ? "Importing…" : "Import for review"}</Button>
          </form>
          <div className="soft-panel rounded-3xl border border-white/80 p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><div><div className="flex items-center gap-2"><ClipboardCheck className="h-4 w-4 text-primary" /><h2 className="font-semibold text-slate-900">Recruiter email review queue</h2></div><p className="mt-1 text-sm text-slate-500">Statuses govern internal review only; they never trigger external communication.</p></div><Badge variant="outline" className="border-slate-200 bg-white">{recruiterEmailEvents.data?.length ?? 0} events</Badge></div><div className="mt-5 space-y-3">{recruiterEmailEvents.isLoading ? <p className="rounded-2xl border border-dashed border-slate-200 bg-white/70 p-6 text-center text-sm text-slate-500">Loading email events…</p> : recruiterEmailEvents.isError ? <div className="rounded-2xl border border-rose-100 bg-rose-50 p-5 text-center"><p className="text-sm text-rose-800">Email events could not be loaded. No data was changed.</p><Button type="button" variant="outline" className="mt-3 bg-white" onClick={() => recruiterEmailEvents.refetch()}>Try again</Button></div> : recruiterEmailEvents.data?.length ? recruiterEmailEvents.data.slice(0, 8).map(event => <div key={event.id} className="rounded-2xl border border-slate-100 bg-white/80 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-medium text-slate-900">{event.subject}</p><p className="mt-1 truncate text-xs text-slate-500">{event.sender} · {formatDate(event.receivedAt)}</p></div><Select value={event.reviewStatus} onValueChange={value => reviewRecruiterEmail.mutate({ eventId: event.id, reviewStatus: value as "reviewed" | "ignored" })}><SelectTrigger className="h-8 w-[118px] bg-white text-xs"><SelectValue /></SelectTrigger><SelectContent>{event.reviewStatus === "unreviewed" ? <SelectItem value="unreviewed" disabled>unreviewed</SelectItem> : null}{recruiterEmailReviewActions.map(status => <SelectItem key={status} value={status}>{status}</SelectItem>)}</SelectContent></Select></div>{event.matchedContactId ? <p className="mt-2 text-xs text-emerald-700">Matched to a verified recruiter contact.</p> : <p className="mt-2 text-xs text-slate-500">No contact match yet; record a verified contact if appropriate.</p>}{event.snippet ? <p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-600">{event.snippet}</p> : null}</div>) : <p className="rounded-2xl border border-dashed border-slate-200 bg-white/70 p-6 text-center text-sm text-slate-500">No imported recruiter emails yet.</p>}</div></div>
        </section>
